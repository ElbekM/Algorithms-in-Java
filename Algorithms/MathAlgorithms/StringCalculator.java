...
public class StringCalculator {
	...
	/**
	 * Returns the result of calculating the string arithmetic expression in infix
	 * notation.
	 */
	...
	private String expressionToRPN(String expression) {
		...
		for (int i = 0; i < expression.length(); ++i) {
			...
			if ((token == '-' && (i == 0)) || ((token == '-') && (expression.charAt(i - 1) != ')')
			...
			if ('(' == token) {
				...
				resultRpn.append(" ");
				while (!stack.empty() && getPriority(token) <= getPriority(stack.peek())) {
